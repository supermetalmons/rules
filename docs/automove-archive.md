...
## 2026-07-10 Pro Promotion and Terminal-Planning Wave

- Objective: produce a stronger Pro automove, clear the retained promotion contract across Pro/Normal/Fast and all variants, then route public Pro to it. No candidate cleared the complete contract, so public runtime source was restored to `frontier_pro_v2_guarded` and every temporary selector was pruned before release work.
- A provisional `frontier_pro_v3_promoted` combined variant-budgeted iterative deepening with guarded root overrides. Its sampled dashboard looked promising, but the required all-variant confirmation failed: Pro `27-21` (`56.25%`), Normal `36-12` (`75%`), and Fast `34-14` (`70.83%`). This invalidated the provisional route.
- Pure search expansion plateaued rather than fixing root choice. Depth-5/56k, depth-5/90k, and depth-6/90k candidates stayed near `5-3` on a four-variant screen; depth-5/56k reached only `11-13` on an all-variant Pro repeat. Broader nodes increased latency without producing the required floor.
- Retained-policy portfolios had useful oracle coverage but no stable router. On repeat 0, guarded and raw each reached `16-8`; a variant portfolio stitched the best observed rows into `22-2`. The same portfolio fell to `15-9` on repeat 1. Exact forced-root analysis found winning sibling/rank alternatives for Offset, Bent, Corner, Split, Swapped, Center, Inner, and Classic states, but phase/rank calibration was not general source.
- The oracle-calibrated v6 selector repaired the measured holdout to `23-1` at about `180ms`, with each targeted repair firing sparsely. It then scored `11-13` on unseen repeat offset 2. This is the clearest evidence from the wave that exact state families, variant phases, and root ranks can fit deterministic seeds while failing fresh openings.
- Online rollout arbitration was also non-promotable. Short Fast/Normal/Pro arbitration and small root pools ranged from roughly `12-12` to `17-7`; deeper or wider policies either rotated losses or exceeded useful latency.
- Stateful deterministic terminal planning exposed a stronger mechanism but not a shippable universal policy. A seven-root Pro-continuation planner with line caching and diversity-first ordering reached `12-0`, confidence `0.9998`, and `669.11ms` average against unseen sampled Pro. The same architecture could not stay within cost after cross-budget deviations.
- A Fast-continuation planner reached `12-0`, confidence `0.9998`, and `83.33ms` against unseen sampled Fast, with `554` cache hits, but generalized poorly: `8-4` at `291.84ms` against Normal and `7-5` at `376.49ms` against Pro. A Normal-continuation planner reached only `9-3` at `344.08ms` against Normal. Dual Fast+Normal and tri-policy planners exceeded the `700ms` cap before producing a promotable result.
- Durable perspective: deterministic full-line rollouts are excellent policy-specific oracles and potential offline labels, but they are not a production root policy by themselves. A future challenger should learn a source-time, opponent-policy-agnostic root score from broad oracle/corpus data, reserve fresh repeat offsets for validation, and only enter runtime after cross-budget and all-variant confirmation.
- Release disposition: no new automove runtime implementation was promoted. The strongest retained and release-safe implementation remains `frontier_pro_v2_guarded`; `shipping_pro_search` remains the baseline; experiment harnesses stay under `#[cfg(test)]` and generated package checks reject diagnostic leakage.
